...
if ($_POST) {
  ...
  if ($_POST['price'] > 0) {
    $values['price'] = (float) $_POST['price'];
  } else {
    $errors['price'] = 'Please enter a valid price';
  }

  if ($_POST['discount'] >= 0 && $_POST['discount'] <= 100) {
    $values['discount'] = (float) $_POST['discount'] / 100.0;
  } else {
    $errors['discount'] = 'please enter a valid discount';
  }

  $values['description'] = $_POST['description'];
  $values['bar_code'] = $_POST['bar_code'];
  $values['category_id'] = $_POST['category_id'];
  $values['color_id'] = $_POST['color_id'];
  $values['size_id'] = $_POST['size_id'];
  $values['is_recent'] = isset($_POST['is_recent']) ? 1 : 0;
    ...
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="col-12">
          ...
          ?>
          <form class="row" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>?id=<?= $product['id'] ?>" method="post" enctype="multipart/form-data">
            <div class="form-group col-6">
              ...
              <div class="form-group col-6">
                <label>Image</label>
                <img src="/uploads/<?= $values['image_url'] ?>" width="250" height="250">
                <input name="image" type="file" />
                <?= isset($errors['image']) ? "<span class='text-danger'>" . $errors['image'] . "</span>" : '' ?>
              </div>
              <div class="form-group col-12">
                <label>Description</label>
                <textarea name="description" class="form-control"><?= isset($values['description']) ? $values['description'] : '' ?></textarea>
              </div>
              <div class="form-group col-6">
                <label>Price</label>
                <input name="price" class="form-control" type="number" min="0" value="<?= isset($values['price']) ? $values['price'] : '' ?>" />
                <?= isset($errors['price']) ? "<span class='text-danger'>" . $errors['price'] . "</span>" : '' ?>
              </div>
              <div class="form-group col-6">
                <label>Discount %</label>
                <input name="discount" type="number" min="0" max="100" class="form-control" value="<?= isset($values['discount']) ? $values['discount']*100 : '' ?>" />
                <?= isset($errors['discount']) ? "<span class='text-danger'>" . $errors['discount']*100 . "</span>" : '' ?>
              </div>
              <div class="form-group col-6">
                <label>Bar Code</label>
                <input name="bar_code" class="form-control" value="<?= isset($values['bar_code']) ? $values['bar_code'] : '' ?>" />
              </div>
              <div class="form-group col-6">
